...
local function SkinPetButton(self)
	...
	self.HotKey:SetPoint("TOPRIGHT", -4, -8)
	self.HotKey:SetFont(C.font.normalFont, 20, C.font.normalFontStyle)
	self.HotKey:SetShadowOffset(C.font.normalFontShadow and 1 or 0, C.font.normalFontShadow and -1 or 0)
	self.HotKey:SetWidth(64)
end
...
hooksecurefunc("PetBattleFrame_UpdateActionBarLayout", function()
	...
	bf.SwitchPetButton:SetSize(64, 64)
	bf.SwitchPetButton:ClearAllPoints()
	bf.SwitchPetButton:SetPoint("LEFT", bf.abilityButtons[3], "RIGHT", 4, 0)
	...
	bf.CatchButton:SetSize(64, 64)
	bf.CatchButton:ClearAllPoints()
	bf.CatchButton:SetPoint("LEFT", bf.SwitchPetButton, "RIGHT", 4, 0)

	bf.ForfeitButton:SetParent(bar)
	bf.ForfeitButton:ClearAllPoints()
	bf.ForfeitButton:SetSize(64, 64)
	bf.ForfeitButton:SetPoint("LEFT", bf.CatchButton, "RIGHT", 4, 0)
...
-- Tooltips skinning
local tooltips = { BattlePetTooltip, PetBattlePrimaryAbilityTooltip, PetBattlePrimaryUnitTooltip,
	FloatingPetBattleAbilityTooltip, FloatingBattlePetTooltip }
...
for _, tt in pairs(tooltips) do
	...
	-- 	R.SkinCloseButton(tt.CloseButton)
	-- end
...
hooksecurefunc("PetBattleUnitFrame_UpdateDisplay", function(self)
	...
	local r, g, b, hex = C_Item.GetItemQualityColor(nQuality)
	...
	elseif self.IconBackdrop then
		self.IconBackdrop:SetBackdropBorderColor(unpack(C.media.borderColor))
	end
