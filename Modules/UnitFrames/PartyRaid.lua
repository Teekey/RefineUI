...
local party_height = C.raidframe.party_height
...
local function Shared(self, unit)
	...
	or (self:GetParent():GetName():match("oUF_Raid")) and "raid" or unit

	local suffix = self:GetAttribute("unitsuffix")

	-- Set our own colors
	self.colors = T.RefineUF_colors
    ...
    self.backdrop.border:SetFrameStrata("MEDIUM")


	self:SetHitRectInsets(-5, -5, -48, -5);
	...
	-- self.Health.mask:SetTexture("Interface\\AddOns\\RefineUI\\Media\\Textures\\MaskTest2.blp", "CLAMPTOWHITEADDITIVE", "CLAMPTOWHITEADDITIVE")
	-- self.Health.mask:SetPoint("TOPLEFT", self.Health, "TOPLEFT", 2, 0)
	-- self.Health.mask:SetPoint("BOTTOMRIGHT", self.Health, "BOTTOMRIGHT", -2, 0)
	...
	elseif unit == "party" then
			...
			-- self.Info:SetPoint("TOP", self.Health, "BOTTOM", 0, 18)
			-- self.Info:SetPoint("TOPRIGHT", self.Health, "TOPRIGHT", 0, 18)
			self.Info:SetPoint("BOTTOM", self.Health, "TOP", 0, 3)
	else
			-- local center = (raid_height - (C.font.unit_frames_font_size * 2 + 2)) / 2
			-- self.Info:SetPoint("TOPLEFT", self.Health, "TOPLEFT", 0, 18)
			-- self.Info:SetPoint("TOPRIGHT", self.Health, "TOPRIGHT", 0, 18)
			self.Info:SetPoint("BOTTOM", self.Health, "TOP", 0, 3)
	end
	...
	if C.raidframe.icons_raid_mark == true then
		...
		self.RaidTargetIndicator:SetPoint("BOTTOMLEFT", self.Health, "TOPLEFT", -6, 0)
	end

	-- LFD role icons
	if C.raidframe.icons_role == true and not (suffix == "target") then
		self.GroupRoleIndicator = self.Health:CreateTexture(GroupRoleIndicator, "OVERLAY")
		self.GroupRoleIndicator:SetSize(16, 16)
		self.GroupRoleIndicator:SetPoint("BOTTOMLEFT", self.Health, "TOPLEFT", -4, 3)
		self.GroupRoleIndicator:SetDrawLayer("BACKGROUND")
	end
	...
	if C.raidframe.icons_ready_check == true and not (suffix == "target" or suffix == "targettarget") then
		...
		self.ReadyCheckIndicator:SetPoint("CENTER", self.Health, 0, 0)
	end

	-- Summon icons
	if C.raidframe.icons_summon == true and not (suffix == "target" or suffix == "targettarget") then
		self.SummonIndicator = self.Health:CreateTexture(nil, "OVERLAY")
		self.SummonIndicator:SetSize(28, 28)
		self.SummonIndicator:SetPoint("CENTER", self.Health, 0, 0)
	end
	...
	if C.raidframe.icons_phase == true and not (suffix == "target" or suffix == "targettarget") then
		...
		self.PhaseIndicator:SetPoint("CENTER", self.Health, 0, 0)
	end
	...
	if C.raidframe.icons_leader == true and not (suffix == "target" or suffix == "targettarget") then
		...
		self.LeaderIndicator:SetPoint("BOTTOMRIGHT", self.Health, "TOPRIGHT", 5, 3)

		-- Assistant icon
		self.AssistantIndicator = self.Health:CreateTexture(nil, "OVERLAY")
		self.AssistantIndicator:SetSize(16, 16)
		self.AssistantIndicator:SetPoint("BOTTOMRIGHT", self.Health, "TOPRIGHT", 5, 3)
	end
	...
	if C.raidframe.plugins_aura_watch == true and not (suffix == "pet" or suffix == "target" or suffix == "targettarget") then
		...
		if C.auras.show_spiral == true then
	...
	if T.PostCreateRaidFrames then
		T.PostCreateRaidFrames(self, unit)
	end
...
----------------------------------------------------------------------------------------
--	Default position of RefineUI unitframes
----------------------------------------------------------------------------------------
oUF:Factory(function(self)
	oUF:RegisterStyle("RefineUIHeal", Shared)
	oUF:SetActiveStyle("RefineUIHeal")
	if C.raidframe.show_party == true then
		local party = self:SpawnHeader("oUF_Party", nil, "custom [@raid6,exists] hide;show",
			...
			"yOffset", C.raidframe.party_vertical and T.Scale(-50),
	...
	if C.raidframe.show_raid == true then
			local raidgroup = self:SpawnHeader("oUF_Raid", nil, "custom [@raid6,exists] show;hide",
				...
				"yOffset", C.raidframe.raid_groups_vertical and T.Scale(-50),
				"point", "TOP",
...
if C.threat.enable then
	...
	if C.actionbars.split_bars then
