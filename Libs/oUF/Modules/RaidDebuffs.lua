...
local UpdateDebuffFrame = function(rd, icon, count, debuffType, duration, expirationTime, spellId)
	if rd.index and rd.type and rd.filter then
		...
		if spellId and R.RaidDebuffsReverse[spellId] then
			rd.reverse = true
		else
			rd.reverse = nil
		end
...
local Update = function(self, _, unit)
	...
	for filter in next, (rd.Filters or auraFilters) do
		...
		while(true) do
			...
			local prio = R.RaidDebuffs[rd.MatchBySpellName and name or spellId]
			if not R.RaidDebuffsIgnore[spellId] and prio and (prio > rd.priority) then
				rd.priority = prio
				rd.index = i
				rd.type = "Custom"
				rd.filter = filter
