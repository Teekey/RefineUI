...
local function SpellName(id)
	...
	if name then
		...
		print("|cffff0000RefineUI: Nameplates spell ID ["..tostring(id).."] no longer exists!|r")
		return "Empty"
	end
end
...
R.DebuffWhiteList = {
	...
	[SpellName(204242)] = true,	-- Quaking Palm (Pandaren)
}

for _, spell in pairs(C.nameplate.debuffsList) do
	R.DebuffWhiteList[SpellName(spell)] = true
end
...
for _, spell in pairs(C.nameplate.buffsList) do
	R.BuffWhiteList[SpellName(spell)] = true
end
...
R.InterruptCast = { -- Yellow border for interruptible cast
	-- Algeth'ar Academy
...
R.ImportantCast = { -- Red border for non-interruptible cast
	-- The Nokhud Offensive
...
local color = C.nameplate.mobColor
